...
import umprime
import matmul_epilogue_pass_of_remove_functions
...
class MatmulEpilogueFusion(abstract_drr.DrrPass):
  ...
  def constraint(self, o, t):
    ...
    map(lambda dst_name: pass_manager.add_pass(
      ir_tools.create_access_topo_drr_one_step_pass(
          matmul_epilogue_pass_of_remove_functions.RemoveDataOpPairPass(src_data_op_name="mm_out", dst_data_op_name=dst_name))), 
          inputs_name_list
    )
    map(lambda dst_name: pass_manager.add_pass(
      ir_tools.create_access_topo_drr_one_step_pass(
          matmul_epilogue_pass_of_remove_functions.RemoveDataOp2SumOp2DataOpPass(src_data_op_name="mm_out", dst_data_op_name=dst_name))), 
          inputs_name_list
    )
  ...
  def _make_index_func_unique_id2index_program(
    ...
    def MatchAndCopyInputIndex(dst_input_name):
        ...
        rm_elementwise_drr_pass = matmul_epilogue_pass_of_remove_functions.RemoveElementInputIndexPass(
        ...
        rm_broadcast_drr_pass = matmul_epilogue_pass_of_remove_functions.RemoveBroadcastInputIndexPass(
            src_data_op_name=anchor_data_op_name,
            dst_load_from_global_op_name=dst_input_name
        )
    ...
    def MatchAndCopyOutputIndex(dst_output_name):
        pass_manager = ir_tools.create_pass_manager()
        removed_programs = MutableList()
        drr_pass = matmul_epilogue_pass_of_remove_functions.RemoveOutputIndexPass(
            src_data_op_name=anchor_data_op_name,
