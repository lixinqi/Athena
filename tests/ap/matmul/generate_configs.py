...
def generate_autotune_wrapper(datatype, num_configs):
    ...
    code_str = autotune_wrapper_template.replace("${datatype}", datatype).replace(
...
def generate_configs(configs_list, config_template_0, config_template):
    ...
    for config in configs_list:
        if config_id == 0:
            ...
            config_code_str = (
                config_template.replace("${tshape}", ", ".join(map(str, config.tshape)))
                .replace("${wshape}", ", ".join(map(str, config.wshape)))
                .replace("${ishape}", ", ".join(map(str, config.ishape)))
                .replace("${stages}", str(config.stages))
...
def main():
    ...
    print(f"-- Total {num_fp16_configs} fp16 configs, {num_fp32_configs} fp32 configs")
