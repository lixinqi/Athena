#include <iostream>

#include "kernel.h"
#include "test_util.h"

template <typename T>
void TestMain(cudaStream_t stream, int batch_count, int m, int n, int k,
              bool add_bias, bool has_epilogue) {
  bool random = true;

  std::vector<int64_t> input_shape{batch_count, m, k};
  std::vector<int64_t> weight_shape{k, n};
  std::vector<int64_t> bias_shape;
  std::vector<int64_t> output_shape{batch_count, m, n};

  T *input = AllocateAndInit<T>(stream, input_shape, random, 1.);
  T *weight = AllocateAndInit<T>(stream, weight_shape, random, 1.);

  T *bias = nullptr;
  if (add_bias) {
    // bias_shape = {n};
    bias_shape = {batch_count, m, n};
    std::vector<float> bias_ref;
    bias_ref.resize(Product(bias_shape));
    for (size_t i = 0; i < bias_ref.size(); ++i) {
      bias_ref[i] = static_cast<float>(1000 * (i % 11));
    }
    bias = AllocateAndInit<T>(stream, bias_shape, random, 0., bias_ref);
  }

  std::vector<const void *> epilogue_ins;
  std::vector<void *> epilogue_outs;
  std::vector<std::vector<int64_t>> epilogue_in_shapes;
  std::vector<std::vector<int64_t>> epilogue_out_shapes;

  T *another = nullptr;
  if (has_epilogue) {
    // std::vector<int64_t> another_shape = output_shape;
    std::vector<int64_t> another_shape = {n};
    std::vector<float> another_ref;
    another_ref.resize(Product(another_shape));
    for (size_t i = 0; i < another_ref.size(); ++i) {
      another_ref[i] = static_cast<float>(10000 * (i % 5));
    }
    another = AllocateAndInit<T>(stream, another_shape, false, 0., another_ref);

    epilogue_ins = {another};
    epilogue_in_shapes = {another_shape};
  }

  T *output = AllocateAndInit<T>(stream, output_shape, false, 0.);
  CHECK_CUDA(cudaStreamSynchronize(stream));

  CHECK_CUDA(
      cudaMemsetAsync(output, 0, sizeof(T) * Product(output_shape), stream));

  KERNEL_PROFILE(ap::MatmulAddBinaryKernel(
      &stream, input, weight, bias, output, epilogue_ins, epilogue_outs,
      input_shape, weight_shape, bias_shape, epilogue_in_shapes,
      epilogue_out_shapes));

  Print<T>(stream, reinterpret_cast<T *>(output), batch_count, m, n);

  cudaFree(input);
  cudaFree(weight);
  if (add_bias) {
    cudaFree(bias);
  }
  cudaFree(output);
}

int main(int argc, const char *argv[]) {
  std::cout << "TestName: test_main_matmul_binary" << std::endl;
  ProblemSizeArgs args = ParseArgs(argc, argv);

  cudaStream_t stream;
  CHECK_CUDA(cudaStreamCreate(&stream));

  bool add_bias = false;
  bool has_epilogue = true;

#if AP_USE_FLOAT16
  TestMain<half>(stream, args.batch_count, args.m, args.n, args.k, add_bias,
                 has_epilogue);
#else
  TestMain<float>(stream, args.batch_count, args.m, args.n, args.k, add_bias,
                  has_epilogue);
#endif

  CHECK_CUDA(cudaStreamDestroy(stream));
  return 0;
}
