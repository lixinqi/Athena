...
template <typename T, int NumIns = 1, int NumOuts = 1> struct AddFunctor {
  struct Arguments {
    int out_shape_len{0};
  ...
  __forceinline__ __host__ __device__ T Load(const Arguments &args,
                                             const MatrixCoord &coord,
                                             int idx) const {
  ...
  __forceinline__ __host__ __device__ T LoadWithBroadcast(
      const Arguments &args, const MatrixCoord &coord, int idx) const {
  ...
  operator()(T x, const Arguments &args, const MatrixCoord &coord) const {
    T y = args.ins[0].need_broadcast ? LoadWithBroadcast(args, coord, 0)
                                     : Load(args, coord, 0);
    return x + y;
  }
};
...
void NativeMatmulAddKernel(cudaStream_t *stream, const void *input,
  ...
  ap::NativeMatmulAdd<float, ap::AddFunctor<float, 1, 1>>(params,
                                                          epilogue_args);
}
}
