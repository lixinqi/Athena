...
template <class VariadicOp, class = void> struct GenericVariadicTraits {
...
struct GenericVariadicTraits<
    VariadicOp, decltype(typename VariadicOp::Arguments(), void())> {
...
public:
  ...
  struct Params {
    ...
    CUTLASS_HOST_DEVICE
    Params()
        : alpha(ElementCompute(1)), beta(ElementCompute(0)), alpha_ptr(nullptr),
          beta_ptr(nullptr) {}
...
public:
  ...
  CUTLASS_HOST_DEVICE
  FragmentOutput operator()(FragmentAccumulator const &accumulator,
                            int row_offset, int column_offset) const {
    CUTLASS_TRACE_DEVICE(
        "kElementsPerAccess: %d, row_offset: %d, column_offset: %d",
        kElementsPerAccess, row_offset, column_offset);
